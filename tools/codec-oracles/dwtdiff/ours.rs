...
fn main() {
    ...
    let scale: u32 = args.get(1).map(|s| s.parse().unwrap()).unwrap_or(4);
    let mut seed: u32 = args.get(2).map(|s| s.parse().unwrap()).unwrap_or(12345);
    let mut buffer = vec![0i16; 4096];
    for v in buffer.iter_mut() {
        seed = seed.wrapping_mul(1103515245).wrapping_add(12345);
        *v = ((seed >> 16) as u16 as i16) >> scale;
    }
    let mut temp = vec![0i16; 4096];
    decode(&mut buffer, &mut temp);
    let mut out = String::new();
    for v in &buffer { out.push_str(&format!("{}\n", v)); }
    print!("{}", out);
}
