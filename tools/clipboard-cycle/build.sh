#!/bin/sh
# Cross-build clipboard-cycle.exe for x86_64-pc-windows-msvc from macOS.
#
# Same recipe as tools/latency-spike/server/build.sh — see that script's header for
# the one-time xwin provisioning:
#
...
# (dxgi, d3d11, mfplat, mfuuid, advapi32 …) and the UCRT. rust-lld speaks link.exe
