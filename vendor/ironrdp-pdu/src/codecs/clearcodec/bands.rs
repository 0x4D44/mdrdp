...
fn decode_vbar<'a>(src: &mut ReadCursor<'a>, band_height: u16) -> DecodeResult<VBar<'a>> {
    ...
    //   bits 7:0   = shortVBarYOn  (8 bits): row where the Short V-Bar begins
    //   bits 13:8  = shortVBarYOff (6 bits): row where the Short V-Bar ends
    // Pixel count = shortVBarYOff - shortVBarYOn
    //
    // mdrdp patch: these two were the other way round — `y_on` was taken from bits 13:6
    // and `y_off` from bits 5:0.
    //
    // That is not a subtle difference. Under the old reading `y_on` ranges to 255 while
    // `y_off` caps at 63, so `y_off < y_on` fires for any y_on above 63 and the whole
    // tile is rejected. Measured against a live Windows host, 84 of 222 ClearCodec
    // commands failed here outright, and the v-bars those tiles would have populated
    // never entered the caches — so a further 92 later tiles failed with "cache miss on
    // hit". Roughly 90% of ClearCodec tiles were lost to this one transposition.
    //
    // FreeRDP reads it as below (libfreerdp/codec/clear.c, SHORT_VBAR_CACHE_MISS):
