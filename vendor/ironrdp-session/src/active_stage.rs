...
impl ActiveStage {
    ...
    pub fn process(
        ...
        // Feed a running bandwidth measurement ([MS-RDPBCGR] 2.2.14) before the frame
        // is dispatched, so the Bandwidth Measure Stop PDU itself is included in the
        // count the same way FreeRDP includes it.
        self.x224_processor.register_inbound_bytes(frame.len());
