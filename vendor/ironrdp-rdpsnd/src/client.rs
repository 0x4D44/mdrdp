...
pub trait RdpsndClientHandler: Send + core::fmt::Debug {
    ...
    /// Reports the exact client format list sent on the wire, in wire order.
    ///
    /// `Wave2Pdu::format_no` indexes this list. The default keeps existing handlers
    /// source-compatible when they do not need to resolve that index themselves.
    fn set_negotiated_formats(&mut self, _formats: &[AudioFormat]) {}
...
impl Rdpsnd {
    ...
    pub fn client_formats(&mut self) -> PduResult<RdpsndSvcMessages> {
        ...
        let server_format: HashSet<_> = self
            ...
            .iter()
            .collect();
        // Preserve handler order: Wave2.format_no indexes this exact vector, so a handler
        // must be able to retain the same order for decoding. HashSet::intersection used
        // to randomize it per process and gave the handler no way to learn the result.
        let formats: Vec<_> = self
            ...
            .cloned()
            .collect();
        self.handler.set_negotiated_formats(&formats);
