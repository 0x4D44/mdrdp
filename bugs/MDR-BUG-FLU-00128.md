...
- **State history:** Open (2026-09-04T22:01:28Z, raised via `deltic bugs new`) -> Fixed (2026-09-05T07:01:55Z, deltic:auto role=fix run=fix-20260905T063843Z-5bd66c1a branch=task/bug-MDR-BUG-FLU-00128-run-fix-20260905T063843Z-5bd66c1a code=da33f3e gate=manual) -> Closed (2026-09-14T11:06:16Z, 0x4D44/Codex verify run=verify-20260914T105004Z-2be849d7)
...
Commands and assertion evidence: [native review fix journal](<~/language/mdrdp/wrk_journals/2026.09.05 - JRN - native review fixes.md>).

## Verification

The independent verifier passed the four auxiliary-startup rollback tests and all 64 `native::session::tests`. The lead passed the same four focused tests and the 64-test native-session family after restoring the fix.

Both verifiers replaced the `AuxStartup` Drop guard at `src/native/session.rs:547` with `if false`. The independent mutant run failed all four focused tests, showing an uncleared local stop, open outbox, and surviving workers. The lead observed three failures and one non-diagnostic pass in the same filter, including the guard, clone-failure, and post-spawn rollback assertions. Restoring the guard made all four focused tests and the native-session family pass again; exact source restoration was verified. No live RDP, Windows, resource-exhaustion, or real-clipboard test was available.
...
Future regression: inject failure at each clone/thread-start boundary; check all started auxiliary workers terminate, the outbox closes, and the main session stop flag remains clear. Include the final poll-spawn failure after the writer has started. The verification above exercises these boundaries.

Low severity reflects the rare OS-resource-failure trigger and one-process-per-session architecture. This does not claim a successful poll thread leaks inside `spawn_aux` (poll creation is its last fallible step), nor that outer session-spawn failure leaks across process exit. Fixed MDR-BUG-FLU-00117 bounds shutdown of successfully constructed channels; this defect occurs before such an owner exists.
