...
- [ ] 2026-08-18: the epilogue dialog's fallback ERRINFO arm shows the reason verbatim, so the detail leads with a raw "[Protocol independent error]" prefix (`~/language/mdrdp/src/disconnect.rs:classify`, `other.description()`). Seen live on the tscon "Another user connected" kick. Strip the bracketed category prefix (or map it to plain words) before display.
- [ ] 2026-08-20: the `--sessions` FPS column grades a raw, unfiltered frame-gap median (`~/language/mdrdp/src/presence.rs:338`, from `SessionStats::frame_gap` whose own doc says idle stretches record as huge gaps and analysis must filter). A genuinely idle session therefore reads FPS 0 in BAD red — measured on a live native session whose `frame_gap_p50_us` was 60,011,134 (the host's one-update-per-minute idle cadence). Honest arithmetic, useless verdict. Either filter to the active-cadence mode before grading, or show idle as "idle" rather than a red zero. Applies to the RDP rows equally.
